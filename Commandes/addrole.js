...
module.exports.run = async(client, message, args) => {
    ...
    let role = message.guild.roles.cache.find(role => role.name === args[1]); //message.guild.roles.find(r => r.name == args[1]) || message.guild.roles.find(r => r.id == args[1]) || message.mentions.roles.first();
    ...
    if (member.roles.cache.has(role.id)) { return message.channel.send('Vous avez déjà ce role !'); }
